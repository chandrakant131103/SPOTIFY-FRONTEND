        ...
        <div style={{ display: 'flex', height: '100vh', width: '100vw', background: '#000' }}>
            <div style={{ 
                ...
                <div style={{ position: 'relative', zIndex: 2 }}>
                    <h1 style={{ fontSize: '84px', fontWeight: '900', margin: 0 }}>PULSE</h1>
            ...
            <div style={{ flex: '0 0 450px', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#0a0a0a', padding: '40px' }}>
                <form onSubmit={handleSubmit} style={{ width: '100%', display: 'flex', flexDirection: 'column' }}>
                    <h2 style={{ fontSize: '32px', marginBottom: '32px' }}>{isLogin ? 'Log In' : 'Create Account'}</h2>
                    {!isLogin && (
                        <>
                            <input name="username" type="text" placeholder="Username" onChange={handleChange} className="auth-input" />
                            <select name="role" onChange={handleChange} className="auth-input" style={{background: '#121212'}}>
                                <option value="user">Listener</option>
                                <option value="artist">Artist</option>
                            </select>
                        </>
                    )}
                    <input name="email" type="email" placeholder="Email" onChange={handleChange} className="auth-input" />
                    <input name="password" type="password" placeholder="Password" onChange={handleChange} className="auth-input" />
                    <button type="submit" className="btn" style={{ marginTop: '10px' }}>{isLogin ? 'LOG IN' : 'SIGN UP'}</button>
                    <p style={{ textAlign: 'center', marginTop: '20px', color: '#1DB954', cursor: 'pointer', fontWeight: 'bold' }} onClick={() => setIsLogin(!isLogin)}>
